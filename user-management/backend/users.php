...
// Get current user from header (in a real app, you'd validate a JWT token)
...
switch($method) {
    case 'GET':
        if (isset($request[0]) && is_numeric($request[0])) {
            getUser($request[0], $currentUserId, $currentUserRole);
        } else {
    ...
    case 'PUT':
        if (isset($request[0]) && is_numeric($request[0])) {
            updateUser($request[0], $currentUserId, $currentUserRole);
        }
        break;
    case 'DELETE':
        if (isset($request[0]) && is_numeric($request[0])) {
            deleteUser($request[0], $currentUserId, $currentUserRole);
        }
        break;
    default:
...
function createUser($currentUserId, $currentUserRole) {
    ...
    $stmt->execute([
        ...
        $input['email'],
        $hashedPassword,
        'user', // New users are regular users by default
        $input['profile']['bio'] ?? '',
        $input['profile']['phone'] ?? '',
...
function updateUser($id, $currentUserId, $currentUserRole) {
    ...
    // Update user
    $stmt = $pdo->prepare("UPDATE users SET name = ?, email = ?, bio = ?, phone = ?, address = ? WHERE id = ?");
    $stmt->execute([
        $input['name'],
        $input['email'],
        $input['profile']['bio'] ?? '',
